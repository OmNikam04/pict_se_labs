...
void translateObject(double tx, double ty, vector<pair<double, double>> &points)
{
    for (int i = 0; i < points.size(); i++)
    {
        points[i].first += tx;
...
void scaleObject(double sx, double sy, double px, double py, vector<pair<double, double>> &points)
{
    for (int i = 0; i < points.size(); i++)
    {
        points[i].first = px + (points[i].first - px) * sx;
        points[i].second = py + (points[i].second - py) * sy;
    }
}
...
void mouseListener(int button, int state, int x, int y)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        translateObject(100, 100, points);
        scaleObject(0.5, 0.5, x, y, points);
